			...
			try {
				...
				try {
					...
					if (presentedToken !== null && !transportAuthenticated) {
						...
						rejectUpgrade(socket, failure.blocked ? 429 : 401, failure.blocked ? "authentication rate limited" : "invalid token", failure.retryAfterMs / 1e3);
						return;
					}
	...
	const handleHealth = async (req, res) => {
		try {
			...
			if (!tokenMatches(requestToken(req), token)) {
				...
				if (failure.blocked) res.setHeader("Retry-After", String(Math.max(1, Math.ceil(failure.retryAfterMs / 1e3))));
				res.end(JSON.stringify({ ok: false }));
				return;
			}
