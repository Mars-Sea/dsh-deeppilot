...
var Dsh012ApiProxy = class {
	...
	sessions = {
		...
		history: async (request) => this.call(async () => {
			...
			const source = inspected.events.filter((event) => typeof event.seq === "number").filter((event) => before === void 0 || Number(event.seq) < before);
			const page = source.slice(Math.max(0, source.length - limit));
			return {
				events: page,
				hasMore: page.length < source.length
			};
		}),
