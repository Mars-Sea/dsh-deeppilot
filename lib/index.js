...
function requiredScope(type) {
	...
	if (type === "c2s.workspace.create" || type === "c2s.session.create" || type === "c2s.session.rename" || type === "c2s.session.archive" || type === "c2s.session.unarchive" || type === "c2s.session.cancel" || type === "c2s.session.selectModel") return "sessions.manage";
	...
	*/
	...
	async openSession(sink, sessionId, tailCount) {
		try {
			...
			if (this.apiProxy.supportsProjections === true) this.refreshProjections(sessionId);
			return true;
		} catch {
			return false;
		}
...
//#endregion
//#region src/dsh012-remote-interactions.ts
/**
...
*/
function hostWireStreamArity(open) {
	return open.length;
}
/**
...
*/
function openHostStream(gateway, endpoint, payload, signal) {
	const open = gateway.wireStream.open;
	if (hostWireStreamArity(open) >= 5) return open(endpoint, payload, void 0, void 0, signal);
	return open(endpoint, payload, signal);
}
function inProcessStream(gateway, endpoint, payload, signal) {
	return (async function* () {
		yield* await openHostStream(gateway, endpoint, payload, signal);
	})();
}
/**
...
*/
async function startDsh012RemoteInteractions(hostCtx, handlers) {
	const connection = hostCtx.get("connection");
	const gateway = hostCtx.get("typertGateway");
	if (connection === void 0) throw new Error("dsh 0.1.2-alpha.3 Host connection is unavailable");
	if (gateway === void 0) throw new Error("dsh 0.1.2-alpha.3 typertGateway is unavailable");
...
var Dsh012ApiProxy = class {
	...
	constructor(ctx, options = {}) {
		...
		if (typeof this.workspaceController?.unarchiveSession !== "function") delete this.workspace?.unarchiveSession;
	...
	*/
	...
	workspace = {
		...
		unarchiveSession: async (request) => this.call(async () => {
			...
			return { archivedSessionIds: [...(await unarchive.call(this.workspaceController, request.payload)).archivedSessionIds] };
		})
	};
	host = {
