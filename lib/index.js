...
const Config = z.object({
	...
	local: z.object({
		enabled: z.boolean().default(true),
		port: z.natural().min(MIN_LOCAL_PORT).max(MAX_LOCAL_PORT).default(DEFAULT_LOCAL_PORT).description("DeepPilot 局域网独立端口（默认 3098，修改后本地连接会短暂重连）")
	}).default({
		enabled: true,
...
*/
function createPhoneServer(handlers) {
...
function apply(ctx, options) {
	...
	*/
	...
	ctx.inject(["webServer"], (sub) => {
		...
		if (enabledNow()) log("/phone compatibility routes registered on DSH web server");
		else log("bridge disabled; /phone refuses connections until re-enabled");
	});
	ctx.effect(() => async () => {
		closeAllConnections();
		const bridge = state.bridge;
