...
function validateRequest(type, value) {
	...
	if (value === void 0 && (type === "c2s.session.create" || type === "c2s.directory.list" || type === "c2s.device.revoke")) value = {};
	...
	if (type === "c2s.schedule.create") {
		if (!validSendId(p.clientRequestId) || !text(p.sessionId) || !text(p.title, 120) || !text(p.prompt, 262144)) return "invalid schedule create fields";
		if ([
			"after_seconds",
			"at",
			"every_seconds",
			...
			"weekly",
			"cron"
		].filter((key) => p[key] !== void 0).length !== 1) return "schedule requires exactly one selector";
		if (p.after_seconds !== void 0 && !integer(p.after_seconds, 1, Number.MAX_SAFE_INTEGER)) return "invalid after_seconds";
		if (p.every_seconds !== void 0 && !integer(p.every_seconds, 60, Number.MAX_SAFE_INTEGER)) return "invalid every_seconds";
		if (p.at !== void 0 && typeof p.at !== "string" && (typeof p.at !== "object" || p.at === null || Array.isArray(p.at))) return "invalid at selector";
	}
	if (type === "c2s.schedule.update") {
		...
		if (!optional("title", (v) => text(v, 120)) || !optional("prompt", (v) => text(v, 262144))) return "invalid schedule update content";
		if (p.change !== void 0 && (!p.change || typeof p.change !== "object" || Array.isArray(p.change))) return "invalid schedule timing change";
	}
...
function requiredScope(type) {
	...
	if (type === "c2s.device.revoke") return void 0;
	...
	if (type === "c2s.workspace.create" || type === "c2s.session.create" || type === "c2s.session.fork" || type === "c2s.session.rename" || type === "c2s.session.archive" || type === "c2s.session.unarchive" || type === "c2s.session.cancel" || type === "c2s.session.selectModel") return "sessions.manage";
...
function scheduleManagementErrorCode(kind) {
	switch (kind) {
	...
	*  frame: a device only receives what its scopes grant (R1/P2). */
	...
	async onMessage(raw) {
		...
		if (this.widgetClient && ![
			...
			if (env.type !== "c2s.device.revoke") return this.fail(env.id, "E_FORBIDDEN", "widget connection is read-only");
		}
		...
		if (env.type.startsWith("c2s.schedule.") && !this.scopes.has("sessions.read")) return this.fail(env.id, "E_FORBIDDEN", "scope sessions.read required");
		...
		switch (env.type) {
			...
			case "c2s.session.fork": {
				const p = env.payload;
				if (!this.deps.bridge.capabilities.sessionFork) return this.fail(env.id, "E_UNSUPPORTED", "session fork unavailable on this host version");
				const result = await this.deps.bridge.forkSession(this.deviceId, {
					sessionId: p.sessionId,
					clientRequestId: p.clientRequestId,
					...p.atSeq !== void 0 ? { atSeq: p.atSeq } : {}
				});
			...
			case "c2s.sessions.archived":
				this.send("s2c.sessions.archived.snapshot", { sessions: this.deps.bridge.listArchivedSessions() }, env.id);
				return;
			case "c2s.session.unarchive": {
				const p = env.payload;
				if (!p?.sessionId) return this.fail(env.id, "E_PROTOCOL", "sessionId required");
				if (!this.deps.bridge.capabilities.sessionRestore) return this.fail(env.id, "E_UNSUPPORTED", "session restore unavailable on this host version");
			...
			case "c2s.schedule.list": {
				...
				if (!result.ok) return this.fail(env.id, scheduleManagementErrorCode(result.kind), result.message);
				this.send("s2c.schedule.snapshot", {
					sessionId: p.sessionId,
					tasks: result.value
				}, env.id);
				return;
			...
			case "c2s.schedule.history": {
				...
				if (!result.ok) return this.fail(env.id, scheduleManagementErrorCode(result.kind), result.message);
				this.send("s2c.schedule.history", {
					sessionId: p.sessionId,
			...
			case "c2s.session.sendPrompt": {
				...
				if (!p?.sessionId || text.trim().length === 0 && rawImages.length === 0 && rawDocuments.length === 0) return this.fail(env.id, "E_PROTOCOL", "sessionId and prompt content required");
				...
				if (rawDocuments.length > 4 || rawImages.length + rawDocuments.length > 4) return this.fail(env.id, "E_PROTOCOL", "too many prompt attachments");
				const images = [];
				...
				for (const document of rawDocuments) {
					if (typeof document?.name !== "string" || typeof document?.mediaType !== "string" || typeof document?.text !== "string" || document.text.length === 0 || document.text.length > 262144) return this.fail(env.id, "E_PROTOCOL", "invalid document attachment");
					const name = sanitizeDocumentField(document.name, 180);
					const mediaType = sanitizeDocumentField(document.mediaType, 120).toLowerCase();
					if (!name || !mediaType || mediaType.startsWith("image/")) return this.fail(env.id, "E_PROTOCOL", "invalid document attachment");
					documents.push({
						name,
						mediaType,
						text: document.text,
			...
			case "c2s.device.revoke": {
				const p = env.payload;
				if (p?.deviceId !== void 0 && typeof p.deviceId !== "string") return this.fail(env.id, "E_PROTOCOL", "deviceId must be a string");
				if (typeof p?.deviceId === "string" && p.deviceId !== this.deviceId) return this.fail(env.id, "E_FORBIDDEN", "deviceId does not match the authenticated device");
				if (this.widgetClient) return this.fail(env.id, "E_FORBIDDEN", "widget connection cannot revoke its device");
...
//#endregion
//#region src/mutation-journal.ts
...
*/
var MutationJournal = class {
	path;
	persistValues;
	entries = Object.create(null);
	inFlight = /* @__PURE__ */ new Map();
	healthy = true;
	constructor(path, persistValues = false) {
		this.path = path;
		this.persistValues = persistValues;
		if (!path || !existsSync(path)) return;
		try {
			if (statSync(path).size > 4194304) throw new Error("oversized mutation journal");
			const parsed = JSON.parse(readFileSync(path, "utf8"));
			if (parsed.version !== 1 || !Array.isArray(parsed.entries) || parsed.entries.length > CAPACITY) throw new Error("invalid mutation journal");
			for (const value of parsed.entries) {
				...
				this.entries[key] = {
					...
					...this.persistValues && entry.value !== void 0 ? { value: entry.value } : {}
				};
			}
		} catch {
			this.healthy = false;
	...
	expired(id, now = Date.now()) {
		const age = now - Number(id.slice(0, 13));
		return age > RETENTION_MS || age < -3e5;
	}
	save() {
		if (!this.path) return;
		mkdirSync(dirname(this.path), {
		...
		try {
			writeFileSync(fd, JSON.stringify({
				version: 1,
				entries: Object.entries(this.entries).map(([key, entry]) => ({
					key,
					...entry
				}))
			}));
	...
	async dispatch(deviceId, id, content, operation) {
		if (!validSendId(id)) return {
			ok: false,
			code: "E_PROTOCOL",
		...
		if (existing !== void 0) {
			...
			if (existing.status === "rejected" && existing.code !== void 0) return {
				ok: false,
				code: existing.code,
				replayed: true
			};
		...
		const now = Date.now();
		for (const [entryKey, entry] of Object.entries(this.entries)) if (now - entry.createdAt > 6051e5 && !this.inFlight.has(entryKey)) delete this.entries[entryKey];
		if (Object.keys(this.entries).length >= CAPACITY) return {
			ok: false,
			code: "E_BUSY",
...
function isMutationErrorCode(value) {
...
function openMutationJournal(path, persistValues = false) {
	if (!path) return new MutationJournal(void 0, persistValues);
	let journal = journals.get(path);
	if (!journal) {
		journal = new MutationJournal(path, persistValues);
...
var HostBridge = class {
	...
	*/
	...
	get capabilities() {
		return {
			...
			schedules: typeof this.apiProxy.schedule?.list === "function" && typeof this.apiProxy.schedule?.create === "function",
	...
	*/
	...
	async forkSession(deviceId, payload) {
		...
		return {
			ok: false,
			kind: scheduleErrorKind(result.code),
	...
	async updateSchedule(deviceId, payload) {
		...
		const result = await this.scheduleMutations.dispatch(deviceId, clientRequestId, request, async () => {
			...
			if (value.updated !== true || value.record === void 0) return {
				ok: false,
				code: scheduleCodeToError(value.code),
...
function scheduleHostError(error) {
	const message = error.message ?? error.code;
	switch (error.code) {
		case "schedule_not_found":
		case "delivery_cursor_not_found": return {
			ok: false,
			kind: "not-found",
			message
		};
		case "schedule_conflict": return {
			ok: false,
			kind: "conflict",
			message
		};
		...
		case "frequency_too_high": return {
			ok: false,
			kind: "invalid",
			message
		};
		case "schedule_ended": return {
			ok: false,
			kind: "invalid",
			message
		};
...
function scheduleMutationError(error) {
	const result = scheduleHostError(error);
	if (result.ok) return {
		ok: false,
		code: "E_INTERNAL",
		message: "unexpected schedule result"
	};
	return {
		ok: false,
		code: result.kind === "not-found" ? "E_NOT_FOUND" : result.kind === "conflict" ? "E_BUSY" : result.kind === "invalid" ? "E_PROTOCOL" : result.kind === "unsupported" ? "E_UNSUPPORTED" : "E_INTERNAL",
		message: result.message
	};
}
function scheduleCodeToError(code) {
...
function scheduleErrorKind(code) {
	switch (code) {
...
var DshApiProxy = class {
	...
	constructor(ctx, options = {}) {
		...
		this.scheduleController = ctx.get("schedule");
		...
		this.scheduleApi = this.scheduleController === void 0 ? void 0 : this.createScheduleApi();
	}
	get schedule() {
...
function createPhoneServer(handlers, tls) {
	...
	const server = tls === void 0 ? createServer$1(onRequest) : createServer({
		key: tls.key,
		cert: tls.cert,
		minVersion: "TLSv1.2"
	}, onRequest);
	...
	*/
			...
			(async () => {
				...
				const candidates = devices.list().filter((device) => {
					...
					if (!mayReceivePush(device, notification)) {
						if (currentConfig().debug === true) log(`push skip "${deviceDisplayName(device)}": notification permission not granted`);
						return false;
					}
					if (registration.categories?.[notification.category] === false) {
						if (currentConfig().debug === true) log(`push skip "${deviceDisplayName(device)}": category ${notification.category} muted`);
						return false;
					}
					return true;
				});
	...
	const reconcileLocal = async () => {
		...
		try {
			...
			server = createPhoneServer(phoneHandlers, {
				key: tls.key,
				cert: tls.cert
			});
			localServer = server;
			...
			if (server !== void 0) {
				if (localServer === server) localServer = void 0;
				await closeServer(server);
			}
	...
	scheduleLocalReconcile = () => {
		localTail = localTail.then(reconcileLocal).catch((error) => log("local reconcile failed: " + String(error)));
	};
	...
	scheduleRemoteReconcile = () => {
		remoteTail = remoteTail.then(reconcileRemote).catch((error) => log("remote reconcile failed: " + String(error)));
	};
	ctx.effect(() => {
		scheduleLocalReconcile?.();
		...
		return async () => {
			...
			await Promise.allSettled([localTail, remoteTail.then(async () => {
