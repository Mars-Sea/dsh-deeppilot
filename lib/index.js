...
var DeviceStore = class DeviceStore {
	...
	static async load(filePath) {
		...
		try {
			...
			if (error?.code === "ENOENT") {} else console.log("[phone-bridge] device registry unreadable, starting empty: " + String(error));
		}
		return store;
	}
	touch(record, now) {
...
*/
var PhoneBridgeReportService = class extends TypertRemoteService {
	snapshot;
	pairingToken;
	rotatePairingToken;
	relayTester;
...
const DEFAULT_REMOTE_HOSTNAME = "dsh-pocket";
/** Preserve custom node names while migrating the pre-rename default. */
function normalizeRemoteHostname(value) {
	const hostname = value?.trim() ?? "";
	if (hostname === "" || hostname.toLowerCase() === "dsh-phone") return DEFAULT_REMOTE_HOSTNAME;
	return hostname;
}
...
function apply(ctx, options) {
	...
	*/
	...
	installSettingsSection(ctx, settingsNamespace("phone-bridge"), Config, normalizeOptions(void 0), {
		setSource: (source) => {
			liveSource = source;
