window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const REPORT_ENDPOINT = "phone-bridge/report";
		/** Explicit, user-triggered endpoint for revealing the pairing secret. */
		const REVEAL_TOKEN_ENDPOINT = "phone-bridge/revealToken";
		/**
		...
		*/
		const ROTATE_TOKEN_ENDPOINT = "phone-bridge/rotateToken";
		...
		const pairingTokenSchema = { parse(value) {
			if (typeof value !== "string" || value.length < 32) throw new TypeError("phone-bridge/revealToken result: invalid token");
			return value;
		} };
		const REPORT_REMOTE_CONTRIBUTION = {
			...
			descriptors: [
				...
				{
					id: `${REPORT_REMOTE_PACKAGE}#phone-bridge/testRelay`,
					service: "phoneBridgeReport",
					namespace: "phone-bridge",
					method: "testRelay",
					invocation: { kind: "direct" },
				...
				{
					id: `${REPORT_REMOTE_PACKAGE}#phone-bridge/testPush`,
					service: "phoneBridgeReport",
					namespace: "phone-bridge",
					method: "testPush",
					invocation: { kind: "direct" },
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				mountReportRemote(anyCtx.remote, () => ctx.get("remote.phone-bridge")).then((mounted) => {
					if (cancelled) {
						mounted.dispose();
						return;
					}
			...
			const setPhoneBridgeEnabled = (value) => {
				enabledStore.set({
					status: "ready",
					enabled: value
				});
			...
			const setPhoneBridgeRemoteEnabled = (value) => {
		...
		function PhoneBridgeSettingsPage(props) {
			...
			try {
				if (typeof props.usePhoneBridgeReport !== "function") {
					...
					const state = props.usePhoneBridgeReport((s) => s);
					report = state.report;
				...
				if (typeof props.usePhoneBridgeRemoteEnabled === "function") {
					const state = props.usePhoneBridgeRemoteEnabled((s) => s);
					remoteEnabled = state.enabled;
					remoteSwitchReady = state.status === "ready";
				} else diag.push("usePhoneBridgeRemoteEnabled hook 缺失");
			...
			return (0, react.createElement)("div", { className: "pbb-section" }, (0, react.createElement)("div", { className: "pbb-row" }, (0, react.createElement)("h2", { className: "pbb-title" }, "DeepPilot"), (0, react.createElement)("button", {
				...
				onClick: () => {
					if (typeof props.setPhoneBridgeEnabled === "function") props.setPhoneBridgeEnabled(!enabled);
				}
				...
				onClick: () => {
					if (typeof props.setPhoneBridgeRemoteEnabled === "function") props.setPhoneBridgeRemoteEnabled(!remoteEnabled);
				}
		...
		//#endregion
		exports.PhoneBridgeSettingsPage = PhoneBridgeSettingsPage;
		exports.apply = apply;
