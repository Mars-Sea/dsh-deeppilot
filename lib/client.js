window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function encodePairingQRPayload(host, grant, tlsFingerprint) {
			const normalizedHost = host.trim();
			if (!validTargetHost(normalizedHost)) throw new TypeError("pairing QR requires a valid HTTPS/WSS host");
			...
			const payload = {
				...
				...tlsFingerprint !== void 0 ? { tlsFingerprint } : {}
			};
		...
		function DeepPilotSettingsPage(props) {
			...
			const showPairingQR = () => {
				...
				props.beginPairing().then(async (grant) => ({
					grant,
					svg: await import_browser.toString(encodePairingQRPayload(pairingTarget.host, grant, pairingTarget.tlsFingerprint), {
						type: "svg",
			...
			}, translateWith(props.t, "panel.tokenAction.copy"))), addressMessage ? (0, react.createElement)("p", { className: "pbb-diag" }, addressMessage) : null, pairingTarget.tlsFingerprint === void 0 ? null : (0, react.createElement)("div", { className: "pbb-row" }, (0, react.createElement)("code", { className: "pbb-token" }, pairingTarget.tlsFingerprint), (0, react.createElement)("button", {
				type: "button",
				className: "pbb-action",
