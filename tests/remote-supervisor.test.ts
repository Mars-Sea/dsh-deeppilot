...
test('remote hostname migrates only the pre-rename default', () => {
  assert.equal(normalizeRemoteHostname(undefined), 'dsh-pocket')
  assert.equal(normalizeRemoteHostname('  '), 'dsh-pocket')
  assert.equal(normalizeRemoteHostname('dsh-phone'), 'dsh-pocket')
  assert.equal(normalizeRemoteHostname('DSH-PHONE'), 'dsh-pocket')
...
test('remote supervisor degrades when the embedded helper is absent', async () => {
  const supervisor = new RemoteSupervisor({
    enabled: true,
    hostname: 'test-phone',
    statePath: '/tmp/dsh-phone-remote-test-state',
    helperPath: '/tmp/dsh-phone-helper-that-does-not-exist',
    log: () => {},
  })
